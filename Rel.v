...
Proof.
  ...
  - apply (le_S (S n) m). exact Hnm.
  - apply (le_S (S n) m'). exact IHHm'o.
Qed.
...
Proof.
  ...
  induction n as [| n' IH].
  - intro H. inversion H.
  - intro H. apply le_S_n in H. apply IH in H. contradiction.
Qed.
(** [] *)
...
Proof.
  ...
  specialize H with (a:=0)(b:=1).
  assert(H01: 0 <= 1).
    { apply (le_S 0 0). apply (le_n 0). }
  apply H in H01.
  apply (le_Sn_n 0) in H01.
  contradiction.
Qed.
(** [] *)
...
Proof.
  ...
  - intros b Hab Hba. 
    destruct b as [| b'].
    + inversion Hab.
    + apply le_S_n in Hab, Hba.
      pose proof (IH b' Hab Hba) as H.
      rewrite H.
      reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  induction Hxy as [ | x x' y Hxx' Hrest IHrest].
  - (* x = y already applied *)
    intros H. exact H.
  - intros H.
    apply IHrest in H.
    apply (rt1n_trans R x x' z).
    + exact Hxx'.
    + exact H.
Qed.
...
Proof.
  ...
  split.
  - intros H.
    induction H as [ | | x y z Hxy IHxy Hyz IHyz].
    + exact (rsc_R X R x y H).
    + exact (rt1n_refl R x).
    + exact (rsc_trans X R x y z IHxy IHyz).
  - intros H.
    induction H as [ | x y z Hxy Hyz IHyz].
    + exact (rt_refl R x).
    + pose proof (rt_step R x y Hxy) as Hxy'.
      exact (rt_trans R x y z Hxy' IHyz).
Qed.
(** [] *)
